...
| `a_refused_write_downgrades_the_connection_once_and_retries_it` | `tests/resource.rs` | The same downgrade on the write loop, which is a second copy of the same control flow. It is listed separately because the read row is not evidence for it: the re-clamp fix landed in both loops and the test in one, and the whole committed suite stayed green with the write half reverted. Mutation-checked the same way: `left: 130048, right: 64511`. No server reached by this campaign can produce the exchange — Samba serves the large chunk, Windows answers short — so the scripted peer is the only proof this path has. |
| `a_writer_handed_back_carries_the_length_its_last_flush_earned` | `tests/resource.rs` | `into_inner()` leaves the adapter by a different path than `poll_flush` does, and the length a completed flush earns is applied in one place both reach. They did not always: `into_writer()` → `write_all()` → `into_inner()` handed back a file reporting a length of zero for acknowledged bytes. The span is a whole number of buffer-fulls so the last flush is still in flight at the hand-back, which is the case the two paths disagreed about. Mutation-checked: `left: 520192, right: 1040384`. |
